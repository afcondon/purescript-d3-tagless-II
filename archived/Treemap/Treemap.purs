...
draw treeJson selector = do
  ...
  setAttributes (unsafeCoerce tileLabels :: D3Selection_ Datum_)
    [ x (DatumFn \d -> node'.x0 (unsafeCoerce d) + 2.0)
    , y (DatumFn \d -> node'.y0 (unsafeCoerce d) + 12.0)
    , text (DatumFn \d -> node'.name (unsafeCoerce d))
    , textAnchor "start"
    , fontSize 10.0
    , fill "#ffffff"
    , fillOpacity (DatumFn \d -> if canShowLabel { minWidth: 30.0, minHeight: 20.0 } (unsafeCoerce d) then 1.0 else 0.0)
    , classed "tile-label"
    ]
