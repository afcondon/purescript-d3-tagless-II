...
import Data.Array (groupBy, length, sortBy, mapWithIndex, concatMap)
...
import PSD3v2.VizTree.Tree (Tree, nestedJoin)
...
prepareRadialData dataPoints =
  let grouped = groupBy (\a b -> a.state == b.state) $ sortBy (compare `on` _.state) dataPoints
  ...
  in Array.mapWithIndex (toStateData maxTotal innerRadius radiusRange) grouped
  where
...
arcPath startAngle endAngle innerRadius outerRadius =
  ...
  in "M" <> show outerStartX <> "," <> show outerStartY <>
     " A" <> show outerRadius <> "," <> show outerRadius <> " 0 " <> show largeArc <> ",1 " <> show outerEndX <> "," <> show outerEndY <>
     " L" <> show innerEndX <> "," <> show innerEndY <>
     " A" <> show innerRadius <> "," <> show innerRadius <> " 0 " <> show largeArc <> ",0 " <> show innerStartX <> "," <> show innerStartY <>
     " Z"
...
drawRadialStackedBar selector populationData = runD3v2M do
  ...
  -- Prepare data
  let states = prepareRadialData populationData
      ...
      makeLegendItem idx age =
        let ageCount = length ageGroups
            yOffset = (toNumber ageCount / 2.0 - toNumber idx - 1.0) * 20.0
        in T.named Group ("legend-" <> age)
  ...
  let tree :: Tree Unit
      ...
      makeStateArcs stateData =
        Array.mapWithIndex (\ageIdx segment ->
          let stateIdx = stateData.stateIndex
