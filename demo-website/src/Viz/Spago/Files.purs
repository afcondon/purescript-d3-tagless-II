...
import PSD3.Data.Node (D3_FocusXY, D3_VxyFxy, D3_XY, NodeID)
...
import Data.String (joinWith, split, take) as String
...
getGraphJSONData { packages, modules, lsDeps, loc } = do
  let
    ...
    stripVersion pkgName =
      let
        parts = String.split (Pattern "-") pkgName
      ...
      packageName = do
        ...
        case root of
          ".spago" ->
            ...
            if packageString == "p" then pieces !! 2 -- .spago/p/aff-7.1.0/hash -> aff-7.1.0 (will be stripped)
    ...
    packageContains = chunk <$>
      ( -- [ (p1, [m1,m4]), (p2, [m2,m3]), (p3, [m5]) ]
        groupBy equalSnd
          $ -- [ [(m1,p1), (m4,p1)], [(m2,p2), (m3,p2)], [(m5,p3)] ]
    ...
    makeNodeFromModuleJSONPL m = do
      ...
      , inSim: true
      , links:
          { targets: (getId <$> m.depends) -- NB these are Module depends
    ...
    makeNodeFromPackageJSONCL p = do
      ...
      , loc: p.loc
      , links:
          { targets: (getId <$> p.depends) -- NB these are Package depends
          , sources: []
          , treeChildren: []
          , inPackage: []
          , outPackage: []
