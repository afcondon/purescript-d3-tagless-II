...
handleAction = case _ of
  Initialize -> do
    log "MapQuartetExample: Generating data"
    quartet <- H.liftEffect MapQuartet.generateMapQuartet
    log "MapQuartetExample: Drawing"
    _ <- H.liftEffect $ eval_D3M $ MapQuartet.drawMapQuartet quartet "#map-quartet-viz"
    log "MapQuartetExample: Complete"
    pure unit
...
render _ =
  HH.div
    ...
    [ HH.header
        ...
        , HH.h1_ [ HH.text "Map Quartet" ]
        , HH.p_ [ HH.text "Four scatter plots demonstrating sparse data with PureScript Maps (15 points out of 200 possible x-values)." ]
        ]
    , HH.section
        [ HP.classes [ HH.ClassName "example-viz-section" ] ]
        [ HH.h2_ [ HH.text "Visualization" ]
        , HH.div
            [ HP.id "map-quartet-viz"
