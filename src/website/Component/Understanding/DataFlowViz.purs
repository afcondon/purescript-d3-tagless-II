...
handleAction = case _ of
  Initialize -> do
    ...
    -- Load Sankey data from CSV via AJAX
    result <- H.liftAff $ AJAX.get ResponseFormat.string "./data/energy.csv"
    case result of
      Left err -> log "DataFlowViz: Failed to load Sankey data"
      Right response -> do
        _ <- H.liftEffect $ eval_D3M $ SankeyViz.draw response.body "div.sankey-viz"
        pure unit
