...
transitionToColorMixing :: forall m. MonadEffect m => SelectionM D3Selection_ m => D3Selection_ -> m Unit
transitionToColorMixing circles = do
  ...
  let transition = transitionWithDuration $ Milliseconds 1500.0
      ...
      colorFn :: Datum_ -> Index_ -> String
      colorFn d _ = case coerceDatumToInt d of
        0 -> "#ff0000"  -- Red
        1 -> "#00ff00"  -- Green
        _ -> "#0000ff"  -- Blue

      -- X position based on datum
      cxFn :: Datum_ -> Index_ -> Number
      cxFn d _ = case coerceDatumToInt d of
        0 -> 30.0  -- Red on left
        1 -> 60.0  -- Green on right
        _ -> 45.0  -- Blue in center

      -- Y position based on datum
      cyFn :: Datum_ -> Index_ -> Number
      cyFn d _ = case coerceDatumToInt d of
