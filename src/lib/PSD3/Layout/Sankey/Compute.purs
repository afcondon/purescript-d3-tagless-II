...
import PSD3.Layout.Sankey.Types (Alignment(..), DependencyMap, Link(..), LinkCSVRow, LinkColorMode(..), LinkID(..), NodeID(..), SankeyConfig, SankeyGraphModel, SankeyLayoutResult, SankeyLink, SankeyNode, defaultSankeyConfig, initialSankeyGraphModel, initialiseSankeyLink, initialiseSankeyNode)

-- Foreign import for Math.pow
...
processCSVLink link = do
  ...
  modify_ _ { linkCount = model.linkCount + 1
            , nodeCount = nodeCount
...
initialiseSankeyNodes = do
  model <- get
  let nodeNames = Set.toUnfoldable $ Map.keys model.nodeIDToName
    ...
    bfsDepth nodes depth current maxDepth
      ...
      | otherwise =
          let
            ...
            nodesWithDepth = map (\node ->
              if Set.member node.index current
              then node { depth = depth }
              else node
            ) nodes
            ...
            next = foldl (\acc node ->
              if Set.member node.index current
              then Set.union acc node.sourceLinks  -- sourceLinks is Set NodeID
              else acc
            ) Set.empty nodes
          in
    ...
    bfsHeight nodes height current maxHeight
      ...
      | otherwise =
          let
            ...
            next = foldl (\acc node ->
              if Set.member node.index current
              then Set.union acc node.targetLinks  -- targetLinks is Set NodeID
              else acc
            ) Set.empty nodes
          in
...
reorderNodesByTopology :: Array SankeyNode -> Array SankeyLink -> Number -> Number -> Array SankeyNode
  ...
  where
    ...
    weightedTargetCenter node allNodes allLinks =
      let
        ...
        targetSum = foldl (\acc link ->
          if link.sourceIndex == node.index
          then
            case find (\n -> n.index == link.targetIndex) allNodes of
              Just targetNode ->
                let center = (targetNode.y0 + targetNode.y1) / 2.0
                in { sum: acc.sum + center * link.value, weight: acc.weight + link.value }
              Nothing -> acc
          else acc
        ) { sum: 0.0, weight: 0.0 } allLinks
      in
        -- For source nodes (no targets), use current position
        if targetSum.weight > 0.0
        then targetSum.sum / targetSum.weight
...
computeNodeLayers nodes config x0 x1 =
  let
    ...
    nodesWithX = map (\node ->
      let x0Pos = x0 + toNumber node.layer * kx
      in node { x0 = x0Pos, x1 = x0Pos + config.nodeWidth }
    ) nodesWithLayer
  in
    nodesWithX
  ...
  where
    ...
    positionLayer ky padding yStart layerNodes =
      let
        ...
        ) { nodes: [], y: yStart, finalY: y1 } layerNodes
...
relaxation nodes links config y0 y1 =
  let
    ...
    relaxed = foldl (\currentNodes i ->
      ...
      in
        ltr
    ) nodes (Array.range 0 (iterations - 1))
  in
    relaxed
  where
    ...
    relaxByLayer currentNodes allLinks padding ascending alpha beta =
      let
        ...
        layerIndices = if ascending
                       then Array.range 0 maxLayer
    ...
    relaxLayer currentNodes allLinks layerIdx padding ascending alpha beta =
      let
        ...
        nodesWithTarget = map (\node ->
          let weightedY = if ascending
                          then calculateWeightedFromSources node currentNodes allLinks  -- left-to-right: look at sources
              ...
              newY0 = node.y0 + dy
              newY1 = node.y1 + dy
          in { node: node { y0 = newY0, y1 = newY1 }, targetY: newY0 }
        ) layerNodes

        -- Sort by target position, but use value as tiebreaker to maintain stable order
        sorted = Array.sortBy (\a b ->
          let posCompare = compare a.targetY b.targetY
          in if posCompare == EQ
             then compare b.node.value a.node.value  -- Larger value first
             else posCompare
        ) nodesWithTarget
          ...
          then
            case find (\n -> n.index == link.targetIndex) allNodes of
              Just targetNode ->
                ...
                in { sum: acc.sum + center * v, totalWeight: acc.totalWeight + v }
              Nothing -> acc
          else acc
        ) { sum: 0.0, totalWeight: 0.0 } allLinks
      in
        if result.totalWeight > 0.0
        then result.sum / result.totalWeight
    ...
    calculateWeightedFromSources node allNodes allLinks =
      let
        ...
        result = foldl (\acc link ->
          if link.targetIndex == node.index
          then
  ...
  where
    ...
    calculateLinkYPositions nodes links =
      let
        ...
        linksWithY0 = foldl (\currentLinks node ->
          let
        ...
        linksWithY1 = foldl (\currentLinks node ->
          let
